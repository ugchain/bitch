...
public interface CoinMapper {
    ...
    @Update(value = "update bitch_coin c set c.blockNumber = #{blockNumber},c.status = #{status} where c.rid = #{rid} and c.type=#{type}")
    void updateBlockNumberAndStatus(@Param(value = "rid") int rid, @Param(value = "blockNumber") int blockNumber, @Param(value = "status") int status, @Param(value = "type") int type);
