...
public class WalletService extends MsgSignable {
     ...
     * 生成address
     */
    public Msg applyAddress(int userId, CoinAttribute coinAttr) {

        BitchWallet bitchWallet = null;
        try {
            bitchWallet = walletMapper.findOne(userId, coinAttr.getRealCoinName());
        } catch (Exception e) {
            log.error("", e);
            return Msg.ERROR;
        }
        ...
        if (bitchWallet != null) {
            return getMsg(Status.OK, getApplyAddressRes(bitchWallet.getUserid(), bitchWallet.getAddress(), coinAttr.getName()));
        }

        bitchWallet = web3jService.createAddress(userId, coinAttr);
        ...
        Map<String, Object> resMap = getApplyAddressRes(userId, bitchWallet.getAddress(), coinAttr.getName());
